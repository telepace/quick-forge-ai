...
### Step 5: Configuring Quick Forge AI

They used the configuration helper to switch to Supabase:

```bash
make configure-db
```

When prompted, they selected option 2 for Supabase and entered their connection URL.
...
Quick Forge AI's flexible database configuration options made this migration straightforward, allowing the team to switch between database options without any changes to their application code. 